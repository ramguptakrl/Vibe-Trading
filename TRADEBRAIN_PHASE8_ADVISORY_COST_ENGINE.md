...
## Resident trader persona vs data credentials

TradeBrain BSE is built for a **resident individual trader**. The canonical BSE cost path is `resident_advisory.py`, which selects/validates resident-individual brokerage economics.

A Zerodha/Kite API key used to obtain backtest, historical, or live market data may belong to an **NRI account**, but that login is represented only as read-only data-credential metadata. Its account type cannot select brokerage, alter DAY/SWING eligibility, or inject NRI product restrictions into the resident trader's advice. No API key/token is stored in the contract.

In short: **data credential identity != target trader persona != execution eligibility**. Lower-level fee utilities may remain reusable, but the canonical TradeBrain BSE resident wrapper rejects a non-resident brokerage persona.
...
- infer the target trader or brokerage from an NRI API/data credential;
- use MTF eligibility as an advice gate;
